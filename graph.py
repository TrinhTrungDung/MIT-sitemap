...
class Digraph(object):
    ...
    def __str__(self):
        ...
        for src in self.edges.keys():
            edges_str.append(str(src))
        edges_str = sorted(edges_str)
        for src in edges_str:
            for dest in range(len(self.edges[Node(src)])):
                result += str(src) + "->" + str(self.edges[Node(src)][dest][0]) + " (" + str(self.edges[Node(src)][dest][1]) + ", " + str(self.edges[Node(src)][dest][2]) + ")\n"
        return result[:-1]
